...
void SelectCmd::execute( vector<string> params )
{
   utile::log.write( LogLevel_Info, "Executing select command" );
   ...
   if( params.size() < 2 )
   {
      ...
      utile::log.write( LogLevel_Debug, "direction '%s'", params[1].c_str() );
   if( !params[1].compare( "u" ) || !params[1].compare( "up" ) )
      ...
      utile::log.write( LogLevel_Warning, "invalid direction '%s'", params[1].c_str() );
}
